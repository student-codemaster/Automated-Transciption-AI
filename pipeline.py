...
def run_pipeline(audio_path, save_output=True, output_dir="final_outputs", use_cache=True, cache_dir=".cache"):
    """Run the complete podcast analysis pipeline with optional caching.
    ...
    try:
        ...
        cache_mgr = get_cache_manager(cache_dir)
        
        # Check cache first if enabled
        if use_cache and cache_mgr.cache_exists(audio_path):
            cached_results = cache_mgr.load_cache(audio_path)
            if cached_results:
                print("Using cached results - skipped processing")
        ...
        print("Processing audio (not in cache)...")
        ...
        # Prepare results tuple
        results = (transcript, topics, sentiment_score, summaries, index, search_engine)
        ...
        if use_cache:
            ...
            cache_mgr.save_cache(audio_path, results, embeddings_data)
