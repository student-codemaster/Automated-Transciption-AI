...
class MultiEpisodeTestRunner:
    ...
    def _generate_observations(self, segments, sentiment_score):
        ...
        if len(segments) < 5:
            observations.append("Few segments detected - content might be short or homogeneous")
        ...
        if unique_kw < 5:
            observations.append("Limited keyword diversity")
        elif unique_kw > 30:
        ...
        if max_duration > 600:
            observations.append("Some very long segments detected - consider refining segmentation")
        ...
        if empty_summaries > len(segments) * 0.3:
            observations.append("Many empty or very short summaries")
        else:
    ...
    def run_batch_test(self, audio_folder):
        ...
        print(f"\n{'='*60}")
        print(f"Multi-Episode Test Runner")
