...
def create_sentiment_trend_with_segments(segments):
    """Create enhanced sentiment trend visualization with segment boundaries and timeline.
    
    Args:
        segments: List of segment dictionaries with 'start', 'end', 'sentiment_score', 'id', 'summary'
    ...
    # Add sentiment line with markers at segment midpoints
    fig.add_trace(go.Scatter(
        x=df['mid_time'],
        y=df['sentiment'],
        mode='lines+markers',
    ...
    for idx, row in df.iterrows():
        ...
        # Add segment label bottom annotation
        fig.add_annotation(
            x=row['mid_time'],
            y=-1.2,
            text=f"Seg {row['id']}<br>[{format_time(row['start'])} - {format_time(row['end'])}]",
            showarrow=False,
            font=dict(size=10),
            xanchor='center',
            yref='paper',
    ...
    fig.add_hline(
        ...
        line_color="green",
        opacity=0.3,
        annotation_text="Positive Zone (>0.5)",
    ...
    fig.add_hline(
        ...
        line_color="red",
        opacity=0.3,
        annotation_text="Negative Zone (<-0.5)",
        annotation_position="right",
        annotation_font_size=10
    )
    fig.add_hline(
    ...
    max_time = df['mid_time'].max() if len(df) > 0 else 1
    min_time = df['mid_time'].min() if len(df) > 0 else 0
    ...
    fig.update_layout(
        title={
            'text': " Sentiment Trend Across Segments<br><sub>X-axis: Time (seconds) | Y-axis: Sentiment Score</sub>",
            'x': 0.5,
            'xanchor': 'center'
        },
...
def create_keyword_cloud(segments, num_keywords=20):
    ...
    fig.suptitle(' Top Keywords & Topics in Episode', fontsize=16, fontweight='bold')
    return fig
...
def create_keyword_bar_chart(segments, num_keywords=15):
    ...
    fig = px.bar(
        ...
        y='keyword',
        orientation='h',
        title=f" Top {num_keywords} Keywords in Episode",
...
def create_keywords_per_segment(segments, top_n_segments=10):
    ...
    fig.update_layout(
        title=f" Top Keywords (First {top_n_segments} Segments)",
        showlegend=False,
